...
If you'd rather not use the [SAILR::Profile](./Profile.html) class to store and/or manage your secrets, you can amend the `use.profile` option:
...
See the [SAILR::Profile](./Profile.html) for information on how to interact with and manage your secrets; but if your intention is to interact with the `Profile` class through your `SAILR::Connection` instance, you can call the `Connection$get.profile()` method:
...
1. Window functions via `SAILDBUtils::getLogin`
    - Removed and replaced by the [SAILR::Profile](./Profile.html) class which handles window logic
