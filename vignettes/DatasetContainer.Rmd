...
# The `SAILR::DatasetContainer` class
...
The `SAILR::DatasetContainer` provides a method of defining a set of datasets to be used in a project to improve inline documentation, reduce issues with undocumented use of datasets and to promote hermiticity to ensure that the same input datsets and configuration produce the same output. The most obvious benefit of this when initially using it will be the ease of updating datasets with their new refreshes.

## Getting Started

### Defining a `SAILR::DatasetContainer`
...
You can define a `SAILR::DatasetContainer` by entering the following:
...
container = DatasetContainer$new(
  ...
  #     available datasets can be found within `SAILR.METADATA`
  #
  ...
  #      the name defined by `SAILR.METADATA` _i.e._ the name of the dataset
...
The `DatasetRef` items referenced above are recorded within the `SAILR.METADATA` constant exported in this package. It contains a list of commonly used datasets with reference to their origin and how their refreshes are constructed; this allows easy, reproducible use.

Please see `SAILR::SAILR.METADATA`, you can print this in your console if you'd prefer to access it via code, or view it through the interface using the `View` function, _e.g._ ...

```{r eval=FALSE}
View(SAILR::SAILR.METADATA)
```
...
#
# NOTE:
#  - The `wlgp.event` table is described by the `SAILR.METADATA`
#    list - see documentation and/or call `View(SAILR.METADATA)`
#
...
#    name
#
#  - The reason for this is that the SAILREF.WIMD2019 is a static reference
...
The following static methods may be used for type validation of `SAILR::DatasetContainer` instances, and can be used to test the names of datasets:

```{r eval=FALSE}
# Check if some object is a `SAILR::DatasetContainer`
#
#  NOTE:
...
# Check if a name is reserved by `SAILR.METADATA`
#
#  Note:
